...
HVOS_ROOT = os.path.dirname(HVOS)  # up from tests/ to hvos/
sys.path.insert(0, os.path.join(HVOS_ROOT, "core", "world_model"))
sys.path.insert(0, os.path.join(HVOS_ROOT, "learning"))
sys.path.insert(0, os.path.join(HVOS_ROOT, "governance"))
sys.path.insert(0, os.path.join(HVOS_ROOT, "reasoning"))
sys.path.insert(0, os.path.join(HVOS_ROOT, "knowledge_graph"))
...
class V10StressTest:
    ...
    def category8_adversarial(self):
        ...
        try:
            ...
            # 验证表还在
            import sqlite3
            conn = sqlite3.connect(rf"{HVOS}\knowledge_graph\kg.db")
            cur = conn.cursor()
